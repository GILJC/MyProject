...
import com.gil.mapper.DelayCountMapperWithCounter;
...
// 복붙하고, 이름만 바꿔서 바로 사용.
									...
									// extends 이후 추가
public class DelayCountWithCounters extends Configured implements Tool{
	public static void main(String[] args) throws Exception { // throws exception 추가
		int res = ToolRunner.run(new Configuration(), new DelayCountWithCounters(), args);
	...
	public int run(String[] args) throws Exception {
		...
		if(otherArgs.length !=2) {
			System.out.println("Usage: DelatCountWithCounters <input> <output>");
			System.exit(2);
		...
		job.setJarByClass(DelayCountWithCounters.class);  //  수정
		job.setMapperClass(DelayCountMapperWithCounter.class);		// 수정
